...
tape( 'the function returns `true` if provided a 64-bit or 128-bit complex number', function test( t ) {
	t.strictEqual( isComplex( new Complex64( 3.0, 2.0 ) ), true, 'returns expected value' );
	t.strictEqual( isComplex( new Complex128( 3.0, 2.0 ) ), true, 'returns expected value' );
	t.end();
});
...
tape( 'the function returns `false` if not provided a complex number', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		t.strictEqual( isComplex( values[i] ), false, 'returns false when provided '+values[i] );
	}
	t.end();
});
